...
def scikit_classifier(model, X, Y, x_online, y_online, features, channels=None, save_model=True, dir_name='None'):
    if channels is None:
    ...
    for x_train, y_train, x_test, y_test in zip(X, Y, x_online, y_online):
...
def scikit_classifier_loocv_csp(model, data: [Window], channels=None, save_model='None', dir_name='csp',
                                prediction='w'):
    if channels is None:
        channels = [0, 1, 2, 3, 4, 5, 6, 7, 8]
